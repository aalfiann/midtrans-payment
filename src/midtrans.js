...
class MidTrans extends Base64 {
  ...
  /**
    * Send Body json object to MidTrans API (Callback-based)
  ...
  /**
   * Send Body json object to MidTrans API (Promise-based)
   * @return {Promise}
   */
  async sendAsync() {
    if(this[_isEmptyConfig]('url')) throw new Error('Action method is required!');
    ...
    return new Promise((resolve, reject) => {
      ...
      if(this.url.endsWith(this.client_key)
        ...
        req = unirest.get(this.url);
      } else if (this.url.includes('/subscriptions/') > 0 && (this.do == 'update')) {
        req = unirest.patch(this.url);
      } else {
        req = unirest.post(this.url);
      }
